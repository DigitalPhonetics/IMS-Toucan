...
if __name__ == '__main__':
    ...
    train_set = FastSpeechDataset(path_to_transcript_dict, train=True, acoustic_model_name="TransformerTTS_LJSpeech/best.pt", cache_dir=cache_dir, lang="en",
                                  min_len_in_seconds=1, max_len_in_seconds=10, device=device, rebuild_cache=False)
    valid_set = FastSpeechDataset(path_to_transcript_dict, train=False, acoustic_model_name="TransformerTTS_LJSpeech/best.pt", cache_dir=cache_dir, lang="en",
                                  min_len_in_seconds=1, max_len_in_seconds=10, device=device, rebuild_cache=False)
    ...
    train_loop(net=model, train_dataset=train_set, valid_dataset=valid_set, device=device, save_directory=save_dir, steps=400000, batch_size=32,
               gradient_accumulation=1, epochs_per_save=10, use_speaker_embedding=False, lang="en", lr=0.01, warmup_steps=8000)
