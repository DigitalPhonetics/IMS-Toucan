...
def continue_training(batchsize=16,
                      ...
                      generator_warmup_steps=200000,
                      epochs_per_save=5,
                      checkpoint_name="checkpoint_1.483_200868.pt"):
    torch.backends.cudnn.benchmark = True
    # we have fixed input sizes, so we can enable benchmark mode
    ...
    g = generator.to(device)
    d = discriminator.to(device)
    g.load_state_dict(torch.load("Models/MelGAN/SingleSpeaker/CSS10/" + checkpoint_name)["generator"])
    d.load_state_dict(torch.load("Models/MelGAN/SingleSpeaker/CSS10/" + checkpoint_name)["discriminator"])
    g.train()
    d.train()
    optimizer_g = torch.optim.Adam(g.parameters())
    optimizer_d = torch.optim.Adam(d.parameters())
    optimizer_g.load_state_dict(
        torch.load("Models/MelGAN/SingleSpeaker/CSS10/" + checkpoint_name)["generator_optimizer"])
    optimizer_d.load_state_dict(
        torch.load("Models/MelGAN/SingleSpeaker/CSS10/" + checkpoint_name)["discriminator_optimizer"])
