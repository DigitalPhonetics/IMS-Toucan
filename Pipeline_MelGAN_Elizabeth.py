...
if __name__ == '__main__':
    ...
    train_set_lj = MelGANDataset(list_of_paths=get_file_list_elizabeth()[:-50],
                                 cache=os.path.join(melgan_cache_dir, "Elizabeth_train.txt"))
    valid_set_lj = MelGANDataset(list_of_paths=get_file_list_elizabeth()[-50:],
                                 cache=os.path.join(melgan_cache_dir, "Elizabeth_valid.txt"))
    ...
    train_loop(batch_size=16,
               ...
               valid_dataset=valid_set_lj,
               device=torch.device("cuda"),
               generator_warmup_steps=100000,
               model_save_dir=model_save_dir,
               checkpoint="checkpoint_476775.pt")
