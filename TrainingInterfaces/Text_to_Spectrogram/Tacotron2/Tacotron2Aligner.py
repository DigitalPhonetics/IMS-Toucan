...
class Tacotron2(torch.nn.Module):
   ...
   """
    ...
    def forward(self,
        ...
        after_outs, before_outs, logits, att_ws, alignments = self._forward(text,
                                                                            text_lengths,
                                                                            speech,
                                                                            speech_lengths,
        ...
        # train the aligner
        align_attn_loss_weight = max(1.0, 15.0 / max((step / 200.0), 1.0))
        align_attn_loss = self.guided_att_loss(att_ws, text_lengths, speech_lengths) * align_attn_loss_weight
        losses["align_diag"] = align_attn_loss.item()
        loss = loss + align_attn_loss
        align_align_loss = self.alignment_loss(att_ws, text_lengths, speech_lengths, step)
        if align_align_loss != 0.0:
            losses["align_align"] = align_align_loss.item()
            loss = loss + align_align_loss
    ...
    def _forward(self,
        ...
        for y in ys.transpose(0, 1):
            att_c, att_w = self.aligner(embedded_text, ilens, y, prev_att_w)
            att_ws = att_ws + [att_w]
    ...
    def inference(self,
        ...
        ilens = text_tensor.new_tensor([text_tensor.unsqueeze(0).size(1)], device=text_tensor.device).long()

        # inference with teacher forcing
        if use_teacher_forcing:
            # this is usually only used for duration extraction
            ...
            if return_align_att:
                outs, _, _, att_ws, alignments = self._forward(text_tensors, ilens, speech_tensors, speech_lengths, return_alignments=return_align_att)
                return outs[0], None, alignments
            else:
                outs, _, _, att_ws = self._forward(text_tensors, ilens, speech_tensors, speech_lengths, return_alignments=return_align_att)
                return outs[0], None, att_ws[0]

        # inference
        h, embedded_text = self.enc.inference(text_tensor, return_text_embed=return_align_att)
