...
class Parselmouth(torch.nn.Module):
    ...
    def forward(self, input_waves, input_waves_lengths=None, feats_lengths=None, durations=None,
                durations_lengths=None, norm_by_average=True, transcript=None):
        ...
        num_frames = [math.ceil(w/self.hop_length) for w in input_waves_lengths] # TODO: adjust frames
        print('input wave lens: ', input_waves_lengths)
        print('Parsel num frames: ', num_frames)
    ...
    def _calculate_f0(self, input, transcript=None):
        x = input.cpu().numpy().astype(np.double)
        snd = parselmouth.Sound(values=x,sampling_frequency=self.fs)
        print('Parselmouth:\n', 'num samples: ', snd.get_number_of_samples(), '\nsampling freq: ', snd.get_sampling_frequency(), '\nsampling period: ', snd.get_sampling_period())
        ...
        print('PARSEL F0 SHAPE: ', f0.shape)
