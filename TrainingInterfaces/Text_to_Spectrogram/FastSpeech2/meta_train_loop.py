...
def train_loop(net,
    ...
    for index, lang in enumerate(["en", "de", "el", "es", "fi", "ru", "hu", "nl", "fr"]):
        ...
        for datapoint in datasets[index][:20]:  # default embedding for plotting is the average embedding of the first 20 datapoints for each language
            if default_embedding is None:
                default_embedding = datapoint[7].squeeze()
            else:
                default_embedding = default_embedding + datapoint[7].squeeze()
        default_embeddings[lang] = (default_embedding / 20).to(device)
