...
def train_loop(net,
    ...
    for step in tqdm(range(step_counter, steps)):
        ...
        while count < batch_size:
            for language_index in random.sample(list(range(len(datasets))), len(datasets)):
                if count < batch_size:
                    # we get one batch for each task (i.e. language in this case) in a randomized order
                    count += 1
                    if language_index not in batches.keys():
                        batches[language_index] = list()
                    try:
                        batch = next(train_iters[language_index])
                        batches[language_index].append(batch)
                    except StopIteration:
                        train_iters[language_index] = iter(train_loaders[language_index])
                        batch = next(train_iters[language_index])
                        batches[language_index].append(batch)

        train_loss = 0.0

        for language_id in batches.keys():
            batch = collate_and_pad(batches[language_id])
            ...
            with autocast():
                if step <= phase_1_steps:
                    ...
                    # We still keep the tasks in different batches because the BatchNorm should stay within language
