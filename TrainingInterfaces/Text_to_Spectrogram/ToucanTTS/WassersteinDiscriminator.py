...
class WassersteinDiscriminator(torch.nn.Module):
    ...
    def _critic_deep_regression_(self, generated_data, real_data, opt_iterations=1):
        ...
        mapping = self._approx_OT_(sol, generated_data.device)
        ...
        target = target.squeeze().to(generated_data.device)
    ...
    def _generator_train_iteration(self, data_generated):
    ...
    def train_step(self, data_generated, data_real):
        self.num_steps += 1
        loss_critic = self._critic_deep_regression_(data_generated.detach(), data_real)
        loss_generator = self._generator_train_iteration(data_generated)
...
class ResNet_D(nn.Module):
    def __init__(self, data_dim, size, nfilter=64, nfilter_max=512, res_ratio=0.1):
        ...
        self.fc = nn.Linear(4000, 1)  # this needs to be changes everytime the window length is changes. It would be nice if this could be done dynamically.
