...
class AlignerDataset(Dataset):
    ...
    def __init__(self,
        ...
        if not os.path.exists(os.path.join(cache_dir, "aligner_train_cache.pt")) or rebuild_cache:
            ...
            self.ap = AudioPreprocessor(input_sr=sr, output_sr=16000, melspec_buckets=80, hop_length=256, n_fft=1024, cut_silence=cut_silences, device=device)
            # build cache
    ...
    def cache_builder_process(self,
        ...
        for path in tqdm(path_list):
            ...
            try:
                cached_text = self.tf.string_to_tensor(transcript, handle_missing=False).squeeze(0).cpu().numpy()
            except KeyError:
                self.tf.string_to_tensor(transcript, handle_missing=True).squeeze(0).cpu().numpy()
            ...
            cached_speech = self.ap.audio_to_mel_spec_tensor(audio=norm_wave, normalize=False, explicit_sampling_rate=16000).transpose(0, 1).cpu().numpy()
