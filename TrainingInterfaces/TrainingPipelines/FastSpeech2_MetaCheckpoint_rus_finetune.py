...
def run(gpu_id, resume_checkpoint, finetune, model_dir, resume, find_faulty_samples_mode=False):
    ...
    resume_checkpoint = "Models/FastSpeech2_Meta_Russian_low_resource/best.pt"

    base_dir = os.path.join("Models", "FastSpeech2_Meta_joint_finetune_russian")
    ...
    datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_css10nl(),
    ...
    datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_fluxsing(),
    ...
    datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_mls_dutch(),
...
def train_loop(net,
    ...
    for i, dataset in enumerate(datasets):
        if i == 0:
            train_loaders.append(DataLoader(batch_size=12,
                                            dataset=dataset,
                                            drop_last=True,
                                            num_workers=2,
                                            pin_memory=True,
            ...
            train_loaders.append(DataLoader(batch_size=2,
                                            dataset=dataset,
                                            drop_last=True,
                                            num_workers=2,
                                            pin_memory=True,
    ...
    default_embeddings = {"ru": None, "en": None, "de": None, "el": None, "es": None, "fi": None, "hu": None, "nl": None, "fr": None}
    for index, lang in enumerate(["ru", "en", "de", "el", "es", "fi", "hu", "nl", "fr"]):
    ...
    for step in tqdm(range(step_counter, steps)):
        ...
        if step % steps_per_checkpoint == 0:
            ...
            for lang in ["ru", "en", "de", "el", "es", "fi", "hu", "nl", "fr"]:
