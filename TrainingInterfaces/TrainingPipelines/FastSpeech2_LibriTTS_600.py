...
def run(gpu_id, resume_checkpoint, finetune, model_dir, resume):
    ...
    if model_dir is not None:
        ...
        save_dir = os.path.join("Models", "FastSpeech2_LibriTTS_600")
    os.makedirs(save_dir, exist_ok=True)

    train_set = prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_libritts_other500(),
                                          corpus_dir=os.path.join("Corpora", "libri_500"),
                                          lang="en",
                                          phone_input=False,
                                          ctc_selection=True)
    ...
    find_faulty_samples(model, train_set, device, "Models/FastSpeech2_LibriTTS/best.pt")

    train_sets = list()
    train_sets.append(train_set)
    train_sets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_libritts(),
