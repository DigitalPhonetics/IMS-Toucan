...
def run(gpu_id, resume_checkpoint, finetune, model_dir, resume):
    ...
    if model_dir is not None:
        ...
        save_dir = os.path.join("Models", "FastSpeech2_Austrian")
    os.makedirs(save_dir, exist_ok=True)

    datasets = list()
    datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_aridialect(),
                                              corpus_dir=os.path.join("Corpora", "aridialect"),
                                              lang="at-lab"))
