...
class HokusPokus_FastSpeech2(torch.nn.Module):

    def __init__(self, device="cpu"):
        super().__init__()
        self.device = device
        self.text2phone = ArticulatoryCombinedTextFrontend(language="de", inference=True)
        self.phone2mel = FastSpeech2(path_to_weights=os.path.join("Models", "FastSpeech2_Hokus_low_res", "best.pt")).to(torch.device(device))
