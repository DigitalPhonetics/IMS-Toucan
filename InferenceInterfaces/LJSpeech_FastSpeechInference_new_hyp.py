...
class FastSpeech2(torch.nn.Module, ABC):
    ...
    def __init__(self,  # network structure related
        ...
        self.load_state_dict(
            torch.load(os.path.join("Models", "FastSpeech2_LJSpeech_new_hyp", "best.pt"), map_location='cpu')["model"])
...
class LJSpeech_FastSpeechInference_new_hyp(torch.nn.Module):
    ...
    def __init__(self, device="cpu", speaker_embedding=None):
        ...
        self.phone2mel = FastSpeech2(idim=133,
                                     odim=80,
                                     spk_embed_dim=None,
                                     encoder_concat_after=True,
                                     decoder_concat_after=True,
                                     duration_predictor_layers=3,
                                     energy_predictor_layers=4,
                                     aheads=6).to(torch.device(device))
