...
from ArticulatoryPapercupTextFrontend import ArticulatoryPapercupTextFrontend

with open("embedding_table.json", 'r', encoding="utf8") as fp:
    datapoints = json.load(fp)
datapoints.pop('̩')

embedding = torch.nn.Sequential(torch.nn.Linear(42, 50),
                                torch.nn.Tanh(),
...
tfe = ArticulatoryPapercupTextFrontend(language='en')
...
torch.save({"embedding_weights": embedding.state_dict()}, "embedding_pretrained_weights_papercup.pt")
