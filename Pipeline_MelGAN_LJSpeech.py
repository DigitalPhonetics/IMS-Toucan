...
if __name__ == '__main__':
    ...
    train_set_lj = MelGANDataset(list_of_paths=get_file_list_ljspeech()[:-50], cache=os.path.join(melgan_cache_dir, "LJSpeech_train.txt"))
    valid_set_lj = MelGANDataset(list_of_paths=get_file_list_ljspeech()[-50:], cache=os.path.join(melgan_cache_dir, "LJSpeech_valid.txt"))
    generator = MelGANGenerator()
    generator.reset_parameters()
    multi_scale_discriminator = MelGANMultiScaleDiscriminator()

    print("Training model")
    train_loop(batch_size=16, steps=2000000, generator=generator, discriminator=multi_scale_discriminator, train_dataset=train_set_lj,
               valid_dataset=valid_set_lj, device=torch.device("cuda"), generator_warmup_steps=100000, model_save_dir=model_save_dir)
