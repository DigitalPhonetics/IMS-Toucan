...
if __name__ == '__main__':
    ...
    train_set_thorsten = MelGANDataset(list_of_paths=get_file_list_thorsten()[:-50], cache=os.path.join(melgan_cache_dir, "thorsten_train.txt"))
    valid_set_thorsten = MelGANDataset(list_of_paths=get_file_list_thorsten()[-50:], cache=os.path.join(melgan_cache_dir, "thorsten_valid.txt"))
    ...
    train_loop(batch_size=16, steps=2000000, generator=generator, discriminator=multi_scale_discriminator, train_dataset=train_set_thorsten,
               valid_dataset=valid_set_thorsten, device=torch.device("cuda"), generator_warmup_steps=100000, model_save_dir=model_save_dir)
