...
class AttLoc(torch.nn.Module):
    ...
    def forward(self,
        ...
        if prior is not None:
            w = w * prior  # calculate posterior mask
...
class AttMultiHeadLoc(torch.nn.Module):
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev, scaling=2.0):
        ...
        for h in range(self.aheads):
            ...
            e = self.gvec[h](
                torch.tanh(self.pre_compute_k[h] + att_conv + self.mlp_q[h](dec_z).view(batch, 1, self.att_dim_k))).squeeze(2)
...
class AttMultiHeadMultiResLoc(torch.nn.Module):
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev):
        ...
        for h in range(self.aheads):
            ...
            e = self.gvec[h](torch.tanh(self.pre_compute_k[h] + att_conv + self.mlp_q[h](dec_z).view(batch, 1, self.att_dim_k))).squeeze(2)
    ...
    if num_encs == 1:
        ...
        if han_mode:
            ...
            for idx in range(num_encs):
                att = initial_att(args.atype[idx],
                                  args.eprojs,
                                  args.dunits,
                                  aheads[idx],
                                  args.adim[idx],
