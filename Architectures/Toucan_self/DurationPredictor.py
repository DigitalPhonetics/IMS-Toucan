...
from Architectures.ToucanTTS_det.ConditionalLayerNorm import ConditionalLayerNorm
...
class DurationPredictor(torch.nn.Module):
    ...
    def __init__(self, idim, n_layers=2, n_chans=384, kernel_size=3, dropout_rate=0.1, offset=1.0, utt_embed_dim=None):
        ...
        self.criterion = DurationPredictorLoss(reduction="none")
    ...
    def compute_loss(self, predicted_durations, gold_durations, text_lengths):
        
        duration_loss = self.criterion(predicted_durations, gold_durations)
        
        duration_masks = make_non_pad_mask(text_lengths).to(predicted_durations.device)
