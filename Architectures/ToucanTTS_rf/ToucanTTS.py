...
from Architectures.ToucanTTS.rectified_flow import RectifiedFlow
...
class ToucanTTS(torch.nn.Module):
    ...
    def __init__(self,
        ...
        print("Gin channels", gin_channels)
        print("prosody_channels", prosody_channels)
        print("utt_embed_dim", utt_embed_dim)
        # the following modules have their own init function, so they come AFTER the init.
        if prosody_order != "all":
            self.duration_predictor = RectifiedFlow(hidden_channels=prosody_channels,
                                                out_channels=1,
            ...
            self.pitch_predictor = RectifiedFlow(hidden_channels=prosody_channels,
                                            out_channels=1,
                                            filter_channels=prosody_channels,
                                            n_heads=1,
            ...
            self.energy_predictor = RectifiedFlow(hidden_channels=prosody_channels,#8
                                            out_channels=1,
                                            filter_channels=prosody_channels,#8
                                            n_heads=1,
            ...
            self.prosody_predictor = RectifiedFlow(hidden_channels=prosody_channels,
                                            out_channels=4,
                                            filter_channels=prosody_channels,
                                            n_heads=1,
                                            n_layers=8,
        ...
        if is_inference:
            ...
            if self.prosody_order != "all":
                ...
                if self.prosody_order=="ped":
                    ...
                    pitch_predictions, _ = self.pitch_predictor(mu=reduced_pitch_space, mask=text_masks.float(), n_timesteps=30, temperature=1.0, c=utterance_embedding)
                ...
                energy_predictions, _ = self.energy_predictor(mu=reduced_energy_space, mask=text_masks.float(), n_timesteps=30, temperature=1.0, c=utterance_embedding)
                ...
                predicted_durations, _ = self.duration_predictor(mu=reduced_duration_space, mask=text_masks.float(), n_timesteps=30, temperature=1.0, c=utterance_embedding)
            ...
            if self.prosody_order != "all":
                ...
                if self.prosody_order == "ped":
                    ...
                    if self.reflow:
                        z1, z0 = self.frozen_pitch_predictor(mu=reduced_pitch_space, mask=text_masks.float(), n_timesteps=30, temperature=1.0, c=utterance_embedding)
                    else:
                        z1 = gold_pitch.transpose(1,2)
                        z0 = None
                ...
                reduced_energy_space = reduced_energy_space.transpose(1, 2)
                if self.reflow:
                        z1, z0 = self.frozen_energy_predictor(mu=reduced_energy_space, mask=text_masks.float(), n_timesteps=30, temperature=1.0, c=utterance_embedding)
                else:
                    z1 = gold_pitch.transpose(1,2)
                    z0 = None
                ...
                reduced_duration_space = reduced_duration_space.transpose(1, 2)
                if self.reflow:
                        z1, z0 = self.frozen_duration_predictor(mu=reduced_duration_space, mask=text_masks.float(), n_timesteps=30, temperature=1.0, c=utterance_embedding)
                else:
                    z1 = transformed_gold_durations
                    z0 = None
