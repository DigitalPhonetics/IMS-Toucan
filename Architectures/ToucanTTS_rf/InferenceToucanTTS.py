...
class ToucanTTS(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if self.prosody_order != "all":
            ...
            self.energy_predictor = RectifiedFlow(hidden_channels=prosody_channels,
                                            out_channels=1,
                                            filter_channels=prosody_channels,
                                            n_heads=1,
                                            ...
                                            gin_channels=utt_embed_dim)
            if reflow:
                self.frozen_duration_predictor = RectifiedFlow(hidden_channels=prosody_channels,
                                                out_channels=1,
                                                filter_channels=prosody_channels,
                                                n_heads=1,
                                                n_layers=duration_predictor_layers,
                                                ...
                                                p_dropout=duration_predictor_dropout_rate,
                                                gin_channels=utt_embed_dim)
                self.frozen_pitch_predictor = RectifiedFlow(hidden_channels=prosody_channels,
                                            out_channels=1,
                                            filter_channels=prosody_channels,
                                            n_heads=1,
                                            n_layers=pitch_predictor_layers,
                                            ...
                                            p_dropout=pitch_predictor_dropout,
                                            gin_channels=utt_embed_dim)
                self.frozen_energy_predictor = RectifiedFlow(hidden_channels=prosody_channels,
                                            out_channels=1,
                                            filter_channels=prosody_channels,
                                            n_heads=1,
                                            n_layers=energy_predictor_layers,
    ...
    def _forward(self,
        ...
        if self.prosody_order != "all":
            ...
            if self.prosody_order == "ped":
                ...
                pitch_predictions, _ = self.pitch_predictor(mu=reduced_pitch_space, mask=text_masks.float(), n_timesteps=30, temperature=prosody_creativity, c=utterance_embedding) if gold_pitch is None else gold_pitch
            ...
            energy_predictions, _ = self.energy_predictor(mu=reduced_energy_space, mask=text_masks.float(), n_timesteps=30, temperature=prosody_creativity, c=utterance_embedding) if gold_energy is None else gold_energy
            ...
            predicted_durations, _ = self.duration_predictor(mu=reduced_duration_space, mask=text_masks.float(), n_timesteps=30, temperature=prosody_creativity, c=utterance_embedding)
