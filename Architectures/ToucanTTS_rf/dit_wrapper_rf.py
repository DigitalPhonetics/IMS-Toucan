...
from Architectures.ToucanTTS.dit import DiTConVBlock


class DitWrapper(nn.Module):
    """ add FiLM layer to condition time embedding to DiT """

    def __init__(self, hidden_channels, out_channels, filter_channels, num_heads, kernel_size=3, p_dropout=0.1, gin_channels=0, gin_mu_channels=0, time_channels=0):
        super().__init__()
        #print("hiddenchannels", hidden_channels)
        #print("out_channels", out_channels)
        self.time_fusion = FiLMLayer(hidden_channels, out_channels, time_channels)
        self.conv1 = ConvNeXtBlock(hidden_channels, out_channels, filter_channels, gin_channels, gin_mu_channels)
        self.conv2 = ConvNeXtBlock(hidden_channels, out_channels, filter_channels, gin_channels, gin_mu_channels)
        self.conv3 = ConvNeXtBlock(hidden_channels, out_channels, filter_channels, gin_channels, gin_mu_channels)
        self.block = DiTConVBlock(hidden_channels, out_channels, hidden_channels, num_heads, kernel_size, p_dropout, gin_channels)

    def forward(self, x, c1,c2, t, x_mask):
        x = self.time_fusion(x, t) * x_mask
        x = self.conv1(x, c1, c2, x_mask)
        x = self.conv2(x, c1, c2, x_mask)
        x = self.conv3(x, c1, c2, x_mask)
        x = self.block(x, c1, c2, x_mask)
        return x
...
class ConvNeXtBlock(nn.Module):
    def __init__(self, in_channels, out_channels, filter_channels, gin_channels, gin_mu_channels):
        super().__init__()
        ...
        self.norm1 = StyleAdaptiveLayerNorm(in_channels + out_channels, gin_channels)
        self.norm2 = StyleAdaptiveLayerNorm(in_channels + out_channels, gin_mu_channels)
        ...
        self.alpha = nn.Parameter(torch.tensor(0.5))  # Learnable weight for phoneme embedding
        self.beta = nn.Parameter(torch.tensor(0.5))   # Learnable weight for speaker embedding

        
    def forward(self, x, c1,c2, x_mask) -> torch.Tensor:
        residual = x
        x = self.dwconv(x) * x_mask
        # Apply separate SALN normalizations
        out1 = self.norm1(x.transpose(1, 2), c1)
        out2 = self.norm2(x.transpose(1, 2), c2)
        # The model outputs depend on these weightings of c1 and c2
        x = self.alpha * out1 + self.beta * out2
...
class Decoder(nn.Module):
    def __init__(self, hidden_channels, out_channels, filter_channels, dropout=0.05, n_layers=1, n_heads=4, kernel_size=3, gin_channels=0, gin_mu_channels=0, with_noise =True):
        super().__init__()
        ...
        if not with_noise:
            out_channels = 0
        self.blocks = nn.ModuleList([DitWrapper(hidden_channels, out_channels, filter_channels, n_heads, kernel_size, dropout, gin_channels, gin_mu_channels, hidden_channels) for _ in range(n_layers)])
