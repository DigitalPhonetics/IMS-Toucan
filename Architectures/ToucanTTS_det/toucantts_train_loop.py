...
from Architectures.ToucanTTS_det.SpectrogramDiscriminator import SpectrogramDiscriminator
from Utility.WarmupScheduler import ToucanWarmupScheduler as WarmupScheduler
from Architectures.ToucanTTS_det.utils import delete_old_checkpoints
from Architectures.ToucanTTS_det.utils import get_most_recent_checkpoint
from Architectures.ToucanTTS_det.utils import plot_progress_spec_toucantts
from Architectures.ToucanTTS_det.run_weight_averaging import average_checkpoints
from Architectures.ToucanTTS_det.run_weight_averaging import get_n_recent_checkpoints_paths
from Architectures.ToucanTTS_det.run_weight_averaging import load_net_toucan
from Architectures.ToucanTTS_det.run_weight_averaging import save_model_for_use
...
def train_loop(net,
    ...
    while True:
        ...
        for batch in tqdm(train_loader):
            ...
            if use_discriminator:
                discriminator_loss, generator_loss = calc_gan_outputs(real_spectrograms=gold_speech,
        ...
        style_embedding_function.eval()
        speech_indexes = train_dataset[0][2].unsqueeze(0)
        for speech_sample in speech_indexes:
                with torch.inference_mode():
                    wave = ap.indexes_to_audio(speech_sample.int().to(device)).detach()
                    mel = spec_extractor.audio_to_mel_spec_tensor(wave, explicit_sampling_rate=16000).transpose(0, 1).detach().cpu()
        ...
        default_embedding = style_embedding_function(
            batch_of_feature_sequences=gold_speech,
            batch_of_feature_sequence_lengths=train_dataset[0][3].to(device)).squeeze()
        torch.save({
