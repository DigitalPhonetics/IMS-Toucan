...
from Architectures.ToucanTTS_det.Conformer import Conformer
from Architectures.ToucanTTS_det.DurationPredictor import DurationPredictor
from Architectures.GeneralLayers.LengthRegulator import LengthRegulator
from Architectures.ToucanTTS_det.PostNet import PostNet
from Architectures.ToucanTTS_det.VariancePredictor import VariancePredictor
from Preprocessing.articulatory_features import get_feature_to_index_lookup
from Architectures.ToucanTTS_det.Glow import Glow
from Architectures.ToucanTTS_det.ToucanTTSLoss import ToucanTTSLoss
...
class ToucanTTS(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 output_spectrogram_channels=128,
    ...
    def _forward(self,
        ...
        if is_inference:
            ...
            energy_predictions = self.energy_predictor(input_energy, padding_mask=None, utt_embed=utterance_embedding)
            embedded_energy_curve = self.energy_embed(energy_predictions.transpose(1, 2)).transpose(1, 2)
            input_duration = encoded_texts + embedded_energy_curve + embedded_pitch_curve
            
            predicted_durations = self.duration_predictor.inference(input_duration, padding_mask=None, utt_embed=utterance_embedding)
            ...
            input_energy = encoded_texts + embedded_pitch_curve
            energy_predictions = self.energy_predictor(input_energy, padding_mask=padding_masks.unsqueeze(-1), utt_embed=utterance_embedding)
            embedded_energy_curve = self.energy_embed(gold_energy.transpose(1, 2)).transpose(1, 2)
            input_duration = encoded_texts + embedded_energy_curve + embedded_pitch_curve
            predicted_durations = self.duration_predictor(input_duration, padding_mask=padding_masks, utt_embed=utterance_embedding)
    ...
    def read_to_file(self,
        ...
        for text in text_list:
            if text.strip() != "":
                ...
                spoken_sentence, sr = self.inference(text)
