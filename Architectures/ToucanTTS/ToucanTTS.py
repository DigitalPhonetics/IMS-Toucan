...
class ToucanTTS(torch.nn.Module):
    ...
    def _forward(self,
        ...
        if is_inference:
            ...
            if run_glow:
                glow_loss = self.flow_matching_decoder.compute_loss(x1=gold_speech.transpose(1, 2), mask=decoder_masks, mu=preliminary_spectrogram.transpose(1, 2), c=utterance_embedding)
            else:
                glow_loss = None
