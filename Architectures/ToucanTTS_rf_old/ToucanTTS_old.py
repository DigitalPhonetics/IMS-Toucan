...
from Utility.utils import dynamic_padding, undo_padding
...
class ToucanTTS(torch.nn.Module):
    ...
    def __init__(self,
        ...
        duration_Unet = Unet(prosody_channels, channels=1)
        self.duration_predictor = RectifiedFlow(duration_Unet)

        pitch_Unet = Unet(prosody_channels, channels=1)
        self.pitch_predictor = RectifiedFlow(pitch_Unet)

        energy_Unet = Unet(prosody_channels, channels=1)
        self.energy_predictor = RectifiedFlow(energy_Unet)
    ...
    def _forward(self,
        ...
        if is_inference:
            ...
            padded_reduced_pitch_space = dynamic_padding(reduced_pitch_space.unsqueeze(1))
            
            pitch_predictions = self.pitch_predictor.sample(noise=padded_reduced_pitch_space).squeeze(1)
            
            pitch_predictions = undo_padding(pitch_predictions, (pitch_predictions.shape[1] - encoded_texts.shape[1], self.prosody_channels - 1))
            ...
            reduced_energy_space = self.energy_latent_reduction(input_energy).unsqueeze(1)
            padded_reduced_energy_space = dynamic_padding(reduced_energy_space)
            energy_predictions = self.energy_predictor.sample(noise=padded_reduced_energy_space).squeeze(1)
            energy_predictions = undo_padding(energy_predictions, (energy_predictions.shape[1] - encoded_texts.shape[1], self.prosody_channels - 1))
            
            embedded_energy_curve = self.energy_embed(energy_predictions.transpose(1, 2)).transpose(1, 2)

            input_durations =  encoded_texts + embedded_energy_curve + embedded_pitch_curve
            reduced_duration_space = self.duration_latent_reduction(input_durations).unsqueeze(1)
            padded_reduced_duration_space = dynamic_padding(reduced_duration_space)
            predicted_durations = self.duration_predictor.sample(noise=padded_reduced_duration_space).squeeze(1)
            
            predicted_durations = undo_padding(predicted_durations, (predicted_durations.shape[1] - encoded_texts.shape[1], self.prosody_channels - 1)).transpose(1,2).squeeze(1)
            ...
            if self.duration_log_scale:
                ...
                transformed_gold_durations = transformed_gold_durations.unsqueeze(-1)
            else:
                transformed_gold_durations = gold_durations.unsqueeze(-1).float()
            reduced_pitch_space = self.pitch_latent_reduction(encoded_texts).unsqueeze(1)
            
            padded_gold_pitch = dynamic_padding(gold_pitch.unsqueeze(1), downsample_width=self.prosody_channels)
            padded_reduced_pitch_space = dynamic_padding(reduced_pitch_space)
            pitch_loss = self.pitch_predictor(padded_reduced_pitch_space, padded_gold_pitch)
            ...
            reduced_energy_space = self.energy_latent_reduction(input_energy).unsqueeze(1)
            padded_gold_energy = dynamic_padding(gold_energy.unsqueeze(1), downsample_width=self.prosody_channels)
            padded_reduced_energy_space = dynamic_padding(reduced_energy_space)
            energy_loss = self.energy_predictor(padded_reduced_energy_space, padded_gold_energy)
            embedded_energy_curve = self.energy_embed(gold_energy.transpose(1, 2)).transpose(1, 2)
            input_durations = encoded_texts + embedded_energy_curve + embedded_pitch_curve
            
            reduced_duration_space = self.duration_latent_reduction(input_durations).unsqueeze(1)
            padded_gold_durations = dynamic_padding(transformed_gold_durations.unsqueeze(1), downsample_width=self.prosody_channels)
            padded_reduced_duration_space = dynamic_padding(reduced_duration_space)
            duration_loss = self.duration_predictor(padded_reduced_duration_space, padded_gold_durations)
