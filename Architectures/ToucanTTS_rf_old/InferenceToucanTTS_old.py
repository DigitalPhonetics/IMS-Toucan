...
class ToucanTTS(torch.nn.Module):
    ...
    def __init__(self,
        ...
        duration_predictor_chans = config.duration_predictor_chans
        ...
        pitch_predictor_chans = config.pitch_predictor_chans
        ...
        energy_predictor_chans = config.energy_predictor_chans
        ...
        self.prosody_channels = config.prosody_channels
        ...
        duration_Unet = Unet(self.prosody_channels, channels=1)
        self.duration_predictor = RectifiedFlow(duration_Unet)

        pitch_Unet = Unet(self.prosody_channels, channels=1)
        self.pitch_predictor = RectifiedFlow(pitch_Unet)

        energy_Unet = Unet(self.prosody_channels, channels=1)
        ...
        if self.prosody_order != "all":
            self.pitch_latent_reduction = torch.nn.Linear(attention_dimension, self.prosody_channels)
            self.energy_latent_reduction = torch.nn.Linear(attention_dimension, self.prosody_channels)
            self.duration_latent_reduction = torch.nn.Linear(attention_dimension, self.prosody_channels)
        else:
            self.prosody_latent_reduction = torch.nn.Linear(attention_dimension, self.prosody_channels)
    ...
    def _forward(self,
        ...
        batch, *data_shape = padded_reduced_pitch_space.shape
        pitch_predictions = self.pitch_predictor.sample(noise=padded_reduced_pitch_space, data_shape=data_shape).squeeze(1)
        ...
        batch, *data_shape = padded_reduced_energy_space.shape
        energy_predictions = self.energy_predictor.sample(noise=padded_reduced_energy_space, data_shape=data_shape).squeeze(1)
        ...
        batch, *data_shape = padded_reduced_duration_space.shape
        predicted_durations = self.duration_predictor.sample(noise=padded_reduced_duration_space, data_shape=data_shape).squeeze(1)
